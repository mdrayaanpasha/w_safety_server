...
import sendWhatsAppMessageTwilio from '../services/whatsapp';
...
export class ComplaintController {
    ...
    static async createComplaint(req: Request, res: Response): Promise<Response> {
        try {
            ...
            if (legalVolunteer?.mobile) {
                await sendWhatsAppMessageTwilio('+918431974713', complaint.id);

            }

            if (policeVolunteer?.mobile) {
                await sendWhatsAppMessageTwilio('+918431974713', complaint.id);

            }

            if (mentalVolunteer?.mobile) {
                await sendWhatsAppMessageTwilio('+918431974713', complaint.id);
